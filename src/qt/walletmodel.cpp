...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered masterbitpos address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 MBPOS.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal masterbitpos:URI (masterbitpos:XyZ...?message=example)
            {
...
static void NotifyzMBPOSReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzMBPOSReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzMBPOSReset.connect(boost::bind(NotifyzMBPOSReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzMBPOSReset.disconnect(boost::bind(NotifyzMBPOSReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zmbposTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
