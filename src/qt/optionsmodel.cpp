...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MBPOS);
    ...
    if (!settings.contains("nAnonymizeMasterBitPOSAmount"))
        settings.setValue("nAnonymizeMasterBitPOSAmount", 1000);

    nAnonymizeMasterBitPOSAmount = settings.value("nAnonymizeMasterBitPOSAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMasterBitPOSAmount"))
        SoftSetArg("-anonymizemasterbitposamount", settings.value("nAnonymizeMasterBitPOSAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in masterbitpos.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterBitPOSAmount:
            return QVariant(nAnonymizeMasterBitPOSAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMasterBitPOSAmount:
            nAnonymizeMasterBitPOSAmount = value.toInt();
            settings.setValue("nAnonymizeMasterBitPOSAmount", nAnonymizeMasterBitPOSAmount);
            emit anonymizeMasterBitPOSAmountChanged(nAnonymizeMasterBitPOSAmount);
            break;
        case CoinControlFeatures:
