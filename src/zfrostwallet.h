...
#ifndef BIMBPOS_ZMBPOSWALLET_H
#define BIMBPOS_ZMBPOSWALLET_H
...
public:
    CzMBPOSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZMBPOS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZMBPOS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BIMBPOS_ZMBPOSWALLET_H
