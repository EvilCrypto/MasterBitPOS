...
CzMBPOSWallet* zwalletMain = NULL;
...
void PrepareShutdown()
{
    ...
    RenameThread("masterbitpos-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "masterbitpos.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "masterbitposd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the MBPOS and zMBPOS money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 23693, 19229));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in MBPOS/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in MBPOS/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, masterbitpos, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in MBPOS/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all MasterBitPOS specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-mbposstake=<n>", strprintf(_("Enable or disable staking functionality for MBPOS inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmbposstake=<n>", strprintf(_("Enable or disable staking functionality for zMBPOS inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:23693"));
    ...
    strUsage += HelpMessageOpt("-backupzmbpos=<n>", strprintf(_("Enable automatic wallet backups triggered after each zMBPOS minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zmbposbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zMBPOS backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizemasterbitposamount=<n>     " + strprintf(_("Keep N MBPOS anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 23694, 19694));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2018-%i The MasterBitPOS Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("masterbitpos-loadblk");
...
/** Sanity checks
 *  Ensure that MasterBitPOS is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. MasterBitPOS Core is shutting down."));
...
#endif
    // Make sure only a single MasterBitPOS process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. MasterBitPOS Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("MasterBitPOS version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //MasterBitPOS: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //MasterBitPOS specific: zerocoin and spork DB's
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of MasterBitPOS Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart MasterBitPOS Core to complete") << "\n";
        ...
        zwalletMain = new CzMBPOSWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zMBPOSWallet
        uiInterface.InitMessage(_("Syncing zMBPOS wallet..."));

        bool fEnableZMbPosBackups = GetBoolArg("-backupzmbpos", true);
        pwalletMain->setZMbPosAutoBackups(fEnableZMbPosBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zmbposTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeMasterBitPOSAmount = 2;
...
//    }
//
//    nAnonymizeMasterBitPOSAmount = GetArg("-anonymizemasterbitposamount", 0);
//    if (nAnonymizeMasterBitPOSAmount > 999999) nAnonymizeMasterBitPOSAmount = 999999;
//    if (nAnonymizeMasterBitPOSAmount < 2) nAnonymizeMasterBitPOSAmount = 2;
    ...
    LogPrintf("Anonymize MasterBitPOS Amount %d\n", nAnonymizeMasterBitPOSAmount);
    ...
    /* Denominations
       ...
       For example:
       1MBPOS+1000 == (.1MBPOS+100)*10
       10MBPOS+10000 == (1MBPOS+1000)*10
    */
